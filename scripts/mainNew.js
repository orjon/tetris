...
$(() => {
  ...
  class Tetrimino {
    ...
    previousPosition() {
      this.shapePrevious = this.shape
    }

    currentPositionClear() {
      for (let i=0; i<this.shape.length; i++) {
        $gridSquares.siblings().eq(this.shape[i]).removeClass('red')
        $gridSquares.siblings().eq(this.shape[i]).removeClass('green')
        $gridSquares.siblings().eq(this.shape[i]).removeClass('blue')
        $gridSquares.siblings().eq(this.shape[i]).removeClass('orange')
        $gridSquares.siblings().eq(this.shape[i]).removeClass('cyan')
        $gridSquares.siblings().eq(this.shape[i]).removeClass('pink')
        $gridSquares.siblings().eq(this.shape[i]).removeClass('purple')
        $gridSquares.siblings().eq(this.shape[i]).addClass('empty')
      }
