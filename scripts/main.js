...
$(() => {
  ...
  function soundSoftNudge() {
    ...
    soundNudgeWav.currentTime = 0;
    soundBumpWav.playbackRate = 0.5;
    soundBumpWav.volumne = 0.2;
    soundNudgeWav.play();
  }
    ...
    deletePreviousLocation() {
      for (let i = 0; i < this.shapePrevious.length; i++) {
        if (this.shapePrevious[i] >= gridShift) {
          $gridSquares
            .siblings()
            .eq(this.shapePrevious[i] - gridShift)
  ...
  function increaseGameSpeed() {
    const newSpeed = 500 - Math.floor(playerScore / gameSpeedMultiplier);
  ...
  function dropRowsAbove(rowsToRemove) {
    for (let i = 0; i < rowsToRemove.length; i++) {
      //loop through all removed lines
      for (
        let tetriNumber = 0;
        tetriNumber < tetriSequence.length;
        tetriNumber++
      ) {
        //loop through all blocks
        tetriSequence[tetriNumber].shapePrevious = tetriSequence[
          tetriNumber
        ].shape.slice();
        for (
          let pixelNumber = 0;
          pixelNumber < tetriSequence[tetriNumber].shape.length;
          pixelNumber++
        ) {
          //loop through all pixels of block
          if (
            tetriSequence[tetriNumber].shape[pixelNumber] <
            rowsToRemove[i] * 10
          ) {
  ...
  function removeRows() {
    for (
      ...
      //loop through all blocks on grid
      tetriSequence[tetriNumber].shapePrevious = tetriSequence[
        tetriNumber
      ].shape.slice();
      const numberPixels = tetriSequence[tetriNumber].shape.length;
      for (
        let pixelNumber = numberPixels - 1;
        pixelNumber >= 0;
        pixelNumber--
      ) {
        //loop through all grid locations occupied by block (backwards)
        if (
          rowsToRemove.includes(
            Math.floor(tetriSequence[tetriNumber].shape[pixelNumber] / 10)
          )
        ) {
          gridLocationsOccupied = removeOccurances(
            gridLocationsOccupied,
            tetriSequence[tetriNumber].shape[pixelNumber]
          ); //Updates list of occupied spaces.
